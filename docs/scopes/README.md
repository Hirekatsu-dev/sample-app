...
管理画面とメンバー画面はアプリケーションを分離しています（DBは1つを共有）。どのスコープがどのアプリに実装されるかは、この対応で読んでください。

| アプリケーション | 役割 | ポート | 対象スコープ |
| --- | --- | --- | --- |
| `apps/frontend` / `apps/api` | メンバー画面 | 8080 / 3000 | 03（メンバー分）、04〜14 |
| `apps/admin_frontend` / `apps/admin_api` | 管理画面 | 8081 / 3001 | 01、02、03（管理者分） |

管理APIは別サーバーのため、エンドポイントのパスに `/admin` プレフィックスは付けません。詳細は @開発方針.md を参照してください。
...
| - | アプリ分離の基盤 | generator の member/admin 2ターゲット化、`apps/admin_api`・`apps/admin_frontend` の土台 |

## 未実装

| # | スコープ | ドキュメント | 前提 |
| --- | --- | --- | --- |
| 01 | 管理者認証 | [01_admin_auth.md](01_admin_auth.md) | - |
